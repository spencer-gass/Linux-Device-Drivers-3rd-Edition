...
#define MODULE_NAME "mutex-scull"
...
struct scull_fifo_dev {
    ...
    char wr_mutex_type;         // 0: semephore, 1: spinlock
    char rd_mutex_type;         // 0: semephore, 1: spinlock
    struct semaphore wsem;      // write mutex semaphore
    struct semaphore rsem;      // read mutex semaphore
    spinlock_t wspinlock;       // write mutex spinlock
    spinlock_t rspinlock;       // read mutex spinlock
